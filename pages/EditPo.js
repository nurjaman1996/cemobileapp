...
export default function AddPo({ route, navigation }) {
  ...
  React.useEffect(() => {
    setDateStart(new Date(data.tanggal_startpo + "T00:00:00.000Z"));
    setDateEnd(new Date(data.tanggal_endpo + "T00:00:00.000Z"));
    setv_kurs(data.kurs);
    setv_overhead(data.overhead_gr);
  ...
  async function EditPo() {
    ...
    if (v_kurs === "" || v_overhead === "" || v_margin === "") {
      ...
      await axios({
        method: "post",
        url: `${process.env.EXPO_PUBLIC_BASE_URL}/dekstop/editpo`,
        data: {
          id_po: data.id_po,
